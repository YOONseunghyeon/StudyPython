...
def setDataIntoDivtbl(conn, tup):
    ...
    cur.close()
    conn.commit()      #COMMIT; 필수요소 없으면 DB에 생성안됌 
...
def deleteDivision(conn, division):
    ...
    cur.execute(query, (division,))  #데이터가 하나면 튜플로 변경하면서 ,를 반드시 넣어줘야함
    conn.commit()
...
if __name__ == '__main__' :
    ...
    # print('정보 입력 성공')
    # 3. membertbl에서 테이터 조회
    # getSomeDataFromMembertbl(scott_con)
    # 4. membertbl에 새 테이터를 입력
    # print('신규 회원 입력')
    # names = input('이름입력:')
